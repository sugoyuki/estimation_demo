import React, { useState, useEffect } from 'react';
import { rulesGeneralApi, servicesApi } from '../api/services';

function RulesGeneral() {
  const [rules, setRules] = useState([]);
  const [services, setServices] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [showForm, setShowForm] = useState(false);
  const [formData, setFormData] = useState({
    service_id: '',
    common_field: '',
    common_number: '',
    resolution: '',
    range1_name: '測定範囲',
    range1_min: '',
    range1_min_unit: '℃',
    range1_min_included: true,
    range1_max: '',
    range1_max_unit: '℃',
    range1_max_included: true,
    range2_name: '',
    range2_min: '',
    range2_min_unit: '',
    range2_min_included: true,
    range2_max: '',
    range2_max_unit: '',
    range2_max_included: true,
    point_fee: 0,
    base_fee: 0,
    is_active: true
  });
  const [editingId, setEditingId] = useState(null);

  useEffect(() => {
    fetchData();
  }, []);

  const fetchData = async () => {
    try {
      setLoading(true);
      const [rulesRes, servicesRes] = await Promise.all([
        rulesGeneralApi.getAll(),
        servicesApi.getAll({ field: '一般' })
      ]);
      setRules(rulesRes.data.data || []);
      setServices(servicesRes.data.data || []);
      setError(null);
    } catch (err) {
      setError('データの取得に失敗しました');
      console.error(err);
    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();

    try {
      const data = {
        ...formData,
        range1_min: formData.range1_min ? parseFloat(formData.range1_min) : null,
        range1_max: formData.range1_max ? parseFloat(formData.range1_max) : null,
        range2_min: formData.range2_min ? parseFloat(formData.range2_min) : null,
        range2_max: formData.range2_max ? parseFloat(formData.range2_max) : null,
        point_fee: parseFloat(formData.point_fee),
        base_fee: parseFloat(formData.base_fee || 0)
      };

      if (editingId) {
        await rulesGeneralApi.update(editingId, data);
      } else {
        await rulesGeneralApi.create(data);
      }
      fetchData();
      resetForm();
    } catch (err) {
      alert('保存に失敗しました: ' + (err.response?.data?.error || err.message));
      console.error(err);
    }
  };

  const handleEdit = (rule) => {
    setFormData({
      service_id: rule.service_id,
      common_field: rule.common_field || '',
      common_number: rule.common_number || '',
      resolution: rule.resolution || '',
      range1_name: rule.range1_name || '測定範囲',
      range1_min: rule.range1_min || '',
      range1_min_unit: rule.range1_min_unit || '℃',
      range1_min_included: rule.range1_min_included !== false,
      range1_max: rule.range1_max || '',
      range1_max_unit: rule.range1_max_unit || '℃',
      range1_max_included: rule.range1_max_included !== false,
      range2_name: rule.range2_name || '',
      range2_min: rule.range2_min || '',
      range2_min_unit: rule.range2_min_unit || '',
      range2_min_included: rule.range2_min_included !== false,
      range2_max: rule.range2_max || '',
      range2_max_unit: rule.range2_max_unit || '',
      range2_max_included: rule.range2_max_included !== false,
      point_fee: rule.point_fee,
      base_fee: rule.base_fee || 0,
      is_active: rule.is_active
    });
    setEditingId(rule.rule_id);
    setShowForm(true);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('このルールを削除してもよろしいですか？')) {
      return;
    }

    try {
      await rulesGeneralApi.delete(id);
      fetchData();
    } catch (err) {
      alert('削除に失敗しました');
      console.error(err);
    }
  };

  const resetForm = () => {
    setFormData({
      service_id: '',
      common_field: '',
      common_number: '',
      resolution: '',
      range1_name: '測定範囲',
      range1_min: '',
      range1_min_unit: '℃',
      range1_min_included: true,
      range1_max: '',
      range1_max_unit: '℃',
      range1_max_included: true,
      range2_name: '',
      range2_min: '',
      range2_min_unit: '',
      range2_min_included: true,
      range2_max: '',
      range2_max_unit: '',
      range2_max_included: true,
      point_fee: 0,
      base_fee: 0,
      is_active: true
    });
    setEditingId(null);
    setShowForm(false);
  };

  const formatRange = (min, max, minUnit, maxUnit, minIncluded, maxIncluded) => {
    if (!min && !max) return 'N/A';
    const minBracket = minIncluded ? '[' : '(';
    const maxBracket = maxIncluded ? ']' : ')';
    const minStr = min ? `${min}${minUnit || ''}` : '-∞';
    const maxStr = max ? `${max}${maxUnit || ''}` : '∞';
    return `${minBracket}${minStr}, ${maxStr}${maxBracket}`;
  };

  if (loading) return <div>読み込み中...</div>;
  if (error) return <div className="error">{error}</div>;

  return (
    <div className="rules-general">
      <div className="page-header">
        <h1>一般分野ルール管理</h1>
        <button onClick={() => setShowForm(!showForm)} className="btn btn-primary">
          {showForm ? 'フォームを閉じる' : '新規追加'}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="form-card">
          <h3>{editingId ? 'ルール編集' : '新規ルール追加'}</h3>

          <div className="form-group">
            <label>サービス *</label>
            <select
              value={formData.service_id}
              onChange={(e) => {
                const selectedService = services.find(s => s.service_id === parseInt(e.target.value));
                setFormData({
                  ...formData,
                  service_id: e.target.value,
                  common_field: selectedService?.common_field || '',
                  common_number: selectedService?.common_number || ''
                });
              }}
              required
            >
              <option value="">選択してください</option>
              {services.map((service) => (
                <option key={service.service_id} value={service.service_id}>
                  {service.common_field && service.common_number &&
                    `[${service.common_field} ${service.common_number}] `}
                  {service.equipment_name}
                </option>
              ))}
            </select>
          </div>


          <div className="form-group">
            <label>分解能</label>
            <input
              type="text"
              value={formData.resolution}
              onChange={(e) => setFormData({ ...formData, resolution: e.target.value })}
              placeholder="例: 0.1℃以下"
            />
          </div>

          <fieldset className="form-section">
            <legend>範囲1</legend>

            <div className="form-group">
              <label>範囲1名称</label>
              <input
                type="text"
                value={formData.range1_name}
                onChange={(e) => setFormData({ ...formData, range1_name: e.target.value })}
              />
            </div>

            <div className="form-row">
              <div className="form-group">
                <label>最小値</label>
                <input
                  type="number"
                  step="any"
                  value={formData.range1_min}
                  onChange={(e) => setFormData({ ...formData, range1_min: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>単位</label>
                <input
                  type="text"
                  value={formData.range1_min_unit}
                  onChange={(e) => setFormData({ ...formData, range1_min_unit: e.target.value })}
                  placeholder="℃"
                />
              </div>
              <div className="form-group">
                <label>
                  <input
                    type="checkbox"
                    checked={formData.range1_min_included}
                    onChange={(e) => setFormData({ ...formData, range1_min_included: e.target.checked })}
                  />
                  含む ({">"}=)
                </label>
              </div>
            </div>

            <div className="form-row">
              <div className="form-group">
                <label>最大値</label>
                <input
                  type="number"
                  step="any"
                  value={formData.range1_max}
                  onChange={(e) => setFormData({ ...formData, range1_max: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>単位</label>
                <input
                  type="text"
                  value={formData.range1_max_unit}
                  onChange={(e) => setFormData({ ...formData, range1_max_unit: e.target.value })}
                  placeholder="℃"
                />
              </div>
              <div className="form-group">
                <label>
                  <input
                    type="checkbox"
                    checked={formData.range1_max_included}
                    onChange={(e) => setFormData({ ...formData, range1_max_included: e.target.checked })}
                  />
                  含む (&lt;=)
                </label>
              </div>
            </div>
          </fieldset>

          <fieldset className="form-section">
            <legend>範囲2（オプション）</legend>

            <div className="form-group">
              <label>範囲2名称</label>
              <input
                type="text"
                value={formData.range2_name}
                onChange={(e) => setFormData({ ...formData, range2_name: e.target.value })}
              />
            </div>

            <div className="form-row">
              <div className="form-group">
                <label>最小値</label>
                <input
                  type="number"
                  step="any"
                  value={formData.range2_min}
                  onChange={(e) => setFormData({ ...formData, range2_min: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>単位</label>
                <input
                  type="text"
                  value={formData.range2_min_unit}
                  onChange={(e) => setFormData({ ...formData, range2_min_unit: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>
                  <input
                    type="checkbox"
                    checked={formData.range2_min_included}
                    onChange={(e) => setFormData({ ...formData, range2_min_included: e.target.checked })}
                  />
                  含む ({">"}=)
                </label>
              </div>
            </div>

            <div className="form-row">
              <div className="form-group">
                <label>最大値</label>
                <input
                  type="number"
                  step="any"
                  value={formData.range2_max}
                  onChange={(e) => setFormData({ ...formData, range2_max: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>単位</label>
                <input
                  type="text"
                  value={formData.range2_max_unit}
                  onChange={(e) => setFormData({ ...formData, range2_max_unit: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>
                  <input
                    type="checkbox"
                    checked={formData.range2_max_included}
                    onChange={(e) => setFormData({ ...formData, range2_max_included: e.target.checked })}
                  />
                  含む (&lt;=)
                </label>
              </div>
            </div>
          </fieldset>

          <div className="form-group">
            <label>点数料金 *</label>
            <input
              type="number"
              value={formData.point_fee}
              onChange={(e) => setFormData({ ...formData, point_fee: e.target.value })}
              step="0.01"
              required
            />
          </div>

          <div className="form-group">
            <label>基本料金 *</label>
            <input
              type="number"
              value={formData.base_fee}
              onChange={(e) => setFormData({ ...formData, base_fee: e.target.value })}
              step="0.01"
              required
            />
          </div>

          <div className="form-group">
            <label>
              <input
                type="checkbox"
                checked={formData.is_active}
                onChange={(e) => setFormData({ ...formData, is_active: e.target.checked })}
              />
              有効
            </label>
          </div>

          <div className="form-actions">
            <button type="submit" className="btn btn-primary">
              {editingId ? '更新' : '追加'}
            </button>
            <button type="button" onClick={resetForm} className="btn">
              キャンセル
            </button>
          </div>
        </form>
      )}

      <div style={{ overflowX: 'auto' }}>
        <table className="data-table">
          <thead>
            <tr>
              <th>ID</th>
              <th>サービス</th>
              <th>分解能</th>
              <th>範囲1名称</th>
              <th>範囲1</th>
              <th>範囲2名称</th>
              <th>範囲2</th>
              <th>点数料金</th>
              <th>基本料金</th>
              <th>有効</th>
              <th>操作</th>
            </tr>
          </thead>
          <tbody>
            {rules.map((rule) => (
              <tr key={rule.rule_id}>
                <td>{rule.rule_id}</td>
                <td>
                  {rule.service ? (
                    <>
                      {rule.service.common_field && rule.service.common_number && (
                        <span style={{ color: '#666', marginRight: '8px' }}>
                          [{rule.service.common_field} {rule.service.common_number}]
                        </span>
                      )}
                      {rule.service.equipment_name}
                    </>
                  ) : 'N/A'}
                </td>
                <td>{rule.resolution || '-'}</td>
                <td>{rule.range1_name || '-'}</td>
                <td>
                  {formatRange(
                    rule.range1_min,
                    rule.range1_max,
                    rule.range1_min_unit,
                    rule.range1_max_unit,
                    rule.range1_min_included,
                    rule.range1_max_included
                  )}
                </td>
                <td>{rule.range2_name || '-'}</td>
                <td>
                  {formatRange(
                    rule.range2_min,
                    rule.range2_max,
                    rule.range2_min_unit,
                    rule.range2_max_unit,
                    rule.range2_min_included,
                    rule.range2_max_included
                  )}
                </td>
                <td>¥{parseFloat(rule.point_fee || 0).toLocaleString()}</td>
                <td>¥{parseFloat(rule.base_fee || 0).toLocaleString()}</td>
                <td>{rule.is_active ? '有効' : '無効'}</td>
                <td>
                  <button onClick={() => handleEdit(rule)} className="btn btn-sm">
                    編集
                  </button>
                  <button
                    onClick={() => handleDelete(rule.rule_id)}
                    className="btn btn-sm btn-danger"
                  >
                    削除
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}

export default RulesGeneral;
